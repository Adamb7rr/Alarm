...
function setAlarm() {
    ...
    alarms.push({ id, time, ringtone: selectedRingtone });

    const alarmList = document.getElementById('alarms');
    const li = document.createElement('li');
    li.setAttribute('id', id);
    li.innerHTML = `${time} - ${selectedRingtone} <button onclick="deleteAlarm(${id})">Delete</button>`;
...
function playAlarmSound(ringtone) {
    ...
    currentAlarmAudio = new Audio(ringtone);
...
function stopAlarmSound() {
    if (currentAlarmAudio) {
        currentAlarmAudio.pause();
        currentAlarmAudio.currentTime = 0;
        isAlertDismissed = true; // Set the flag to stop ringing
